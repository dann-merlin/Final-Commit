...
float noise( in vec3 x )
{
	...
	return 1. - 0.82*mix( 1.0f, 1.0f, f.z );
}
...
{
    float n = -mod(time * 20 * 0.1,-2.); // noise amount
    float iter = 2.0;
    for (int i = 0; i < 8; i++)
    {
...
float map(vec3 p) 
{
	float mouse_mv = 0.5f;//iMouse.x/iResolution.x;
	R(p.xz, mouse_mv*0.008*pi+time * 20 *0.1);
...
{  
	time = iTime * 0.00075f;
	time = (iTime + 250)* 0.002f;
	if(iTime < 1250 || iTime > 1750) return;
	...
	coord.x = map_val(tc.x, 0, 1, -x_offset, aspect - x_offset);
	// if(coord.x < 0 || coord.x > 1) discard;
	coord += vec2(-0.3f, -0.2f);
    
    vec2 uv = tc;
	// ro: ray origin
	// rd: direction of the ray
	vec3 rd = normalize(vec3(coord * 2 - 1, 1.));
	...
	#else
    fragColor = vec4(sum.xyz, 0.8f * length(sum.xyz));
	#endif
}
