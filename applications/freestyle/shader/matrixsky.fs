...
const float STRIP_CHAR_WIDTH = 0.08;
const float STRIP_CHAR_HEIGHT = 1.5f * STRIP_CHAR_WIDTH;
const float ZCELL_SIZE = 1. * (STRIP_CHAR_HEIGHT * STRIP_CHARS_MAX);  //the multiplier can't be less than 1.
const float XYCELL_SIZE = 80. * STRIP_CHAR_WIDTH;  //the multiplier can't be less than 1.
...
vec3 rain(vec3 ro3, vec3 rd3, float time) {
	// ro3.xy /= iResolution;
	// return vec3(rd3.x,rd3.y,0) * 0.5f;
    vec4 result = vec4(0.);
    ...
    for (int i=0; i<ITERATIONS; i++) {
        ...
        for (int j=0; j<2; j++) {  //2 iterations is enough if camera doesn't look much up or down
			...
			// if(target.y/100 <= 0.01) return vec3(0,1,0);
			// return vec3(target.y)/100;
...
void main()
{
	...
	if(plane_id == 0) {
		...
		coords = vec2(-1,1) * out_pos.zy;
	} else if(plane_id == 2) {
		coords = out_pos.zy; 
	} else if(plane_id == 3) {
		coords = vec2(-1,1) * out_pos.xy;
	} else {
	...
	time += 5 * plane_id;
	vec2 uv = ((coords) * 0.5f * iResolution.xy) / iResolution.y;
	time *= SPEED;
	// time *= 1000;
	// time = int(time) % 5000;
	// time /= 1000;
	...
	float alpha = 1.f;
	if(coords.y < -0.7) {
		float tmp = 0 + (1.f / (-0.7f + 1)) * (coords.y + 1);
		tmp *= tmp;
		alpha = tmp;
	} else if (coords.y > 0.8) {
		float tmp = (1.f / (0.8f - 1)) * (coords.y - 1);
		tmp *= tmp;
		alpha = tmp;
	}
    fragColor = vec4(alpha * col, 1);
}
