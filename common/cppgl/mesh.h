...
public:
	...
	Mesh(const std::string &name, std::string animation_name, const aiMesh *mesh_ai);
	...
	virtual void load_animation(const aiScene *scene_ai, std::string animation_name, bool first = false, bool default_attack = false);

	virtual void do_animation(std::string anim_id);
...
template<class... Args>
std::shared_ptr<Mesh> make_mesh(Args &&... args) {
