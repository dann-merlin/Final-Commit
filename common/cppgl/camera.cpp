...
float Camera::default_camera_movement_speed = 0.02;

Camera::Camera(const std::string &name) : NamedMap(name), pos(0, 1, 0), dir(1, 0, 0), up(0, 1, 0),
										  fov_degree(70), near(0.1), far(15000), left(-100), right(100),
										  bottom(-100), top(100),
...
void Camera::update() {
	...
	proj = perspective ? glm::perspective(fov_degree * float(M_PI / 180), aspect_ratio(), near, far) : glm::ortho(left,
																												  right,
																												  bottom,
																												  top,
																												  near,
...
void Camera::default_mouse_handler(double dt_ms) {
	// mouse
	static float rot_speed = 0.003;
	static glm::vec2 last_pos(-1);
	const glm::vec2 curr_pos = Context::mouse_pos();
	...
	// this will limit the camera movement in order to stop it from flipping when looking up/down (still flips when moving the mouse fast)
	current()->pitch(dt_ms * diff.y * rot_speed);
	while (glm::dot(current()->dir, current()->up) < -0.99 || glm::dot(current()->dir, current()->up) > 0.99){
		current()->pitch(dt_ms * -diff.y * 0.1 * rot_speed);
	}
...
bool Camera::isVisible(glm::vec3 point) {
	auto cam = Camera::find("mapcam");
	float angle = glm::radians(cam->fov_degree) / 2;
	glm::vec3 vertical = -normalize(cam->pos - glm::vec3(cam->pos.x, 0, point.z));
	glm::vec3 horizontal = -normalize(cam->pos - glm::vec3(point.x, 0, cam->pos.z));
	float angle_vertical = acos(dot(vertical, cam->dir));
	float angle_horizontal = acos(dot(horizontal, cam->dir));
	/* std::cout << angle_horizontal << " " << angle_vertical << " " << angle << std::endl; */
	return angle_vertical < angle && angle_horizontal < angle;
}
