...
void Context::stop_running() {
	glfwSetWindowShouldClose(instance().glfw_window, 1);
}
...
double Context::frame_time() { return time_pass * (instance().curr_t - instance().last_t); }
...
bool Context::key_released(int key) { return glfwGetKey(instance().glfw_window, key) == GLFW_RELEASE; }
