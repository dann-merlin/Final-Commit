...
Material::Material(const std::string& name, const std::string& base_path, const aiMaterial* mat_ai) : Material(name) {
	...
	if (mat_ai->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
		...
		add_texture("diffuse", make_texture(name + "_" + get_filename(path_ai.C_Str()) + "_diffuse", path_ai.C_Str()));
	...
	if (mat_ai->GetTextureCount(aiTextureType_SPECULAR) > 0) {
		...
		add_texture("specular", make_texture(name + "_" + get_filename(path_ai.C_Str()) + "_specular", path_ai.C_Str()));
	...
	if (mat_ai->GetTextureCount(aiTextureType_HEIGHT) > 0) {
		...
		add_texture("normalmap", make_texture(name + "_" + get_filename(path_ai.C_Str()) + "_normalmap", path_ai.C_Str()));
	}
	if (mat_ai->GetTextureCount(aiTextureType_OPACITY) > 0) {
		aiString path_ai;
		mat_ai->GetTexture(aiTextureType_OPACITY, 0, &path_ai);
		add_texture("alphamap", make_texture(name + "_" + get_filename(path_ai.C_Str()) + "_alphamap", path_ai.C_Str()));
	}
