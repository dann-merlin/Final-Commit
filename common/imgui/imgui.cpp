...
float ImGui::GetBestCalcTextSize(const char* text, float wished_width, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
	ImGuiContext& g = *GImGui;
	...
	ImFont* font = g.Font;
	float font_size_bak = g.FontSize;
	g.FontSize = 64;
	...
	ImVec2 text_size;
	do {
		text_size = font->CalcTextSizeA(font_size, FLT_MAX, wrap_width, text, text_display_end, NULL);
		font_size -= 1;
	} while(text_size.x > wished_width);
	g.FontSize = font_size_bak;
	return font_size + 1;
}
