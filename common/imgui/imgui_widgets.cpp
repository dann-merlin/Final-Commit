...
bool ImGui::ShrinkingButton(const char* label, const ImVec2& size_arg) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImVec2 label_size;
	// label_size = GetBestCalcTextSize(label, NULL, true);
	...
	// RenderTextWrapped(bb.Min + style.FramePadding, label, NULL, bb.Max.x - bb.Min.x);
